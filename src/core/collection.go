...
type CollectionConfig struct {
	...
	Datas map[string]data.Config `json:"datas"`
...
func (c *Collection) UpdateConfig() (config *CollectionConfig, err error) {

	// Get all datas configurations from imports
	config = c.config
	return
}
...
// collection
func (c *Collection) RefreshDatasConfig() {

	c.config.Datas = make(map[string]data.Config)

	// For each import
	for _, i := range c.imports {

		// For each data generated by each import
		for _, importData := range i.GetDataList() {
			c.refreshDataConfig(importData, make(map[data.Ref]struct{}))
		}
	}
}

func (c *Collection) refreshDataConfig(d data.Data, alreadyExisting map[data.Ref]struct{}) {

	// Get data ref name
	refName := d.GetRef().String()

	// Does the data ref already exist
	if _, ok := c.config.Datas[refName]; ok {
		return
	}

	// Store data config if it exists
	if hasConfig, ok := d.(data.HasConfig); ok {
		if config := hasConfig.GetConfig(); config != nil {
			c.config.Datas[refName] = config
		}
	}
	...
	if hasDeps, ok := d.(data.HasDependencies); ok {
		...
		for _, dep := range hasDeps.GetDependencies() {
			...
			c.refreshDataConfig(dep, alreadyExisting)
		}
	}
}
